...
/** Shared right inset style for content that must stay clear of the floating sidepanel. */
export const getRightInsetStyle = (rightInsetPx = 0) =>
  rightInsetPx > 0 ? { paddingRight: `${rightInsetPx}px` } : undefined;

/** Shared left inset style for content that must stay clear of the floating sidebar. */
...
export const getFloatingPanelReserveWidth = (
  panelWidth: number,
  gutterPx = FLOATING_PANEL_GUTTER_PX,
) => panelWidth + (gutterPx * 2);

export const getFloatingPanelSurfaceMaxWidth = (gutterPx = FLOATING_PANEL_GUTTER_PX) =>
  `calc(100vw - ${gutterPx * 2}px)`;

export const getFloatingPanelLayout = (
  side: 'left' | 'right',
  panelWidth: number,
  gutterPx = FLOATING_PANEL_GUTTER_PX,
) => {
  const g = `${gutterPx}px`;
  const top    = `max(${g}, env(safe-area-inset-top))`;
  const bottom = g;
  const left   = `max(${g}, env(safe-area-inset-left))`;
  const right  = `max(${g}, env(safe-area-inset-right))`;

  return {
    containerStyle: side === 'left'
      ? { top, bottom, left }
      : { top, bottom, right },
    surfaceStyle: {
      width: panelWidth,
      maxWidth: getFloatingPanelSurfaceMaxWidth(gutterPx),
    },
    reserveWidth: getFloatingPanelReserveWidth(panelWidth, gutterPx),
  };
};
